...
class BarrierNode(Node):
    def __init__(self, ip_address: str, port: int, node_name: str, debug: bool):
        ...
        super().__init__(ip_address, 5555, node_name, None)
    ...
    def main_loop(self):
        try:
            try:
                ...
                if self.debug: print("{} - Socket error {}".format(self.get_time(), e))
                sys.exit()
            finally:
                if self.debug: print(
                    "{} - Successfully connect to IP:{}, PORT:{}".format(self.get_time(), self.ip_address, self.port))
    ...
    def parse_socket_data(self, data: str):
        ...
        if data == "IS_ALIVE":
            self.socket_write("ACK", "IS_ALIVE")
            last_ping = time.time()
        if data == "KERING_STATUS":
            self.socket_write("KERING_STATUS", str(self.barrier_open))
    ...
    def socket_write(self, data: str, data_header: str):
        ...
        if self.debug: print("{} - Client send: {}".format(self.get_time(), message))
    ...
    def socket_read(self):
        ...
        if self.debug: print("{} - Client received: {}".format(self.get_time(), data))
        if (data[0] == self.uuid) or (data[0] == "BROADCAST"):
            return self.parse_socket_data(data[1])
